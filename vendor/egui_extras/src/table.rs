...
impl<'a> TableBuilder<'a> {
    ...
    /// Set the horizontal scroll offset position, in points.
    #[inline]
    pub fn horizontal_scroll_offset(mut self, offset: f32) -> Self {
        self.scroll_options.scroll_offset_x = Some(offset);
        self
    }

    /// Enable or disable the scroll area's built-in vertical wheel and bar input.
    /// The vertical viewport remains active when disabled, allowing a caller to
    /// drive it with an external scrollbar via [`Self::vertical_scroll_offset`].
    #[inline]
    pub fn native_vertical_input(mut self, enabled: bool) -> Self {
        self.scroll_options.native_vertical_input = enabled;
        self
    }
    ...
    pub fn header(self, height: f32, add_header_row: impl FnOnce(TableRow<'_, '_>)) -> Table<'a> {
        ...
        let horizontal_offset = scroll_options
            .scroll_offset_x
            .unwrap_or(state.horizontal_offset)
            .max(0.0);
        let header_content_width = state.column_widths.iter().sum::<f32>()
            + ui.spacing().item_spacing.x * state.column_widths.len().saturating_sub(1) as f32;
        ...
        ScrollArea::horizontal()
            ...
            .show_viewport(ui, |ui, _viewport| {
                ...
                header_rect.max.y = header_rect.min.y + height;

                let mut ui_builder = egui::UiBuilder::new().max_rect(header_rect);
                if is_sizing_pass {
...
struct TableState {
    ...
    /// Horizontal body offset used to keep the fixed header synchronized.
...
impl Table<'_> {
        ...
        F: for<'b> FnOnce(TableBody<'b>),
    {
        ...
        let viewport_width = available_width
            + if vscroll {
        ...
        // Wide data tables need both axes in the same viewport. This keeps the
        // horizontal bar fixed beneath the visible rows while scrolling down.
        let scroll_source = if native_vertical_input {
            ScrollSource {
                drag: drag_to_scroll,
                ..Default::default()
            }
        ...
        let mut scroll_area = ScrollArea::new([true, vscroll])
            ...
            .wheel_scroll_multiplier(egui::vec2(
                1.0,
                if native_vertical_input { 3.0 } else { 0.0 },
            ))
            .scroll_source(scroll_source)
            .stick_to_bottom(stick_to_bottom)
            .min_scrolled_height(min_scrolled_height)
            .max_height(max_scroll_height)
            .auto_shrink([false, auto_shrink.y])
        ...
        if let Some(scroll_offset_x) = scroll_offset_x {
            scroll_area = scroll_area.horizontal_scroll_offset(scroll_offset_x);
        }

        let columns_ref = &columns;
        let widths_ref = &state.column_widths;
        let content_width = widths_ref.iter().sum::<f32>()
            + ui.spacing().item_spacing.x * widths_ref.len().saturating_sub(1) as f32;
        let max_used_widths_ref = &mut max_used_widths;

        let scroll_area_out = scroll_area.show_viewport(ui, move |ui, _viewport| {
            // `StripLayout` positions cells directly, so it does not otherwise
            // advertise the complete row width to a horizontal ScrollArea.
            // Explicitly reporting it gives the bar a real range and handle.
            ui.set_min_width(content_width);
            let mut scroll_to_y_range = None;

            let clip_rect = ui.clip_rect();
            let body_rect = ui.available_rect_before_wrap();
            let mut ui_builder = egui::UiBuilder::new().max_rect(body_rect);
            if is_sizing_pass {
                ui_builder = ui_builder.sizing_pass();
            }
            ui.scope_builder(ui_builder, |ui| {
        ...
        // translate them explicitly to stay attached to their columns.
        let mut x = cursor_position.x - scroll_area_out.state.offset.x - spacing_x * 0.5;
            ...
            *column_width = width_range.clamp(*column_width);
            ...
            if column.is_auto() && (is_sizing_pass || !column_is_resizable) {
                *column_width = width_range.clamp(max_used_widths[i]);
                ...
                let table_clip_rect = ui.clip_rect();
                let scroll_bar_hit_width = ui.spacing().scroll.bar_width
                    + ui.spacing().scroll.bar_inner_margin
                    + ui.spacing().scroll.bar_outer_margin;
                let resize_clip_rect = egui::Rect::from_min_max(
                    table_clip_rect.min,
                    egui::pos2(
                        table_clip_rect.max.x - scroll_bar_hit_width,
                        table_clip_rect.max.y,
                    ),
                );
                let line_rect = egui::Rect::from_min_max(p0, p1)
                    .expand(ui.style().interaction.resize_grab_radius_side)
                    // Column resize handles are registered after the scroll bars.
                    // Without clipping, a handle for a wide/off-screen column can
                    // overlap the vertical bar and steal its drag interaction.
                    .intersect(resize_clip_rect);

                let pointer_over_vertical_bar = vscroll
        ...
        state.horizontal_offset = scroll_area_out.state.offset.x;
...
impl<'a> TableBody<'a> {
    ...
    /// Render a virtual row window selected by an explicit external offset.
    /// Unlike [`Self::rows`], this does not infer the first row from an
    /// enclosing [`ScrollArea`], and is intended for caller-owned scrollbars.
    pub fn rows_at_offset(
        mut self,
        row_height_sans_spacing: f32,
        total_rows: usize,
        offset_y: f32,
        ...
        let spacing = self.layout.ui.spacing().item_spacing;
        let row_height_with_spacing = row_height_sans_spacing + spacing.y;
        let visible_rows = (self.y_range.span() / row_height_with_spacing).ceil() as usize + 1;
        let requested_first = (offset_y.max(0.0) / row_height_with_spacing).floor() as usize;
        let first_row = requested_first.min(total_rows.saturating_sub(visible_rows));
