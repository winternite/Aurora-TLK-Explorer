...
pub fn create_winit_window_attributes(
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        // in a Wayland session normally provide XDG_ACTIVATION_TOKEN. Aurora
        // deliberately uses XWayland on Wayland for working file drops, so
        // accept either token and attach it to the X11 window. KWin can then
        // associate the surface with the launch context and choose the correct
        // output instead of falling back to the primary monitor.
